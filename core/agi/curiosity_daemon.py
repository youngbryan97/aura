...
class AutonomousCuriosityDaemon:
    ...
    def __init__(self, tracker: Optional[EpistemicTracker] = None, interval_seconds: int = 300):
        ...
        while self._is_running:
            try:
                ...
                if profile.gaps:
                    ...
                    resolved_engine = capability_engine or ServiceContainer.get("capability_engine", default=None)
                    resolved_gate = will_gate or ServiceContainer.get("authority_gateway", default=None)
                    ...
                    if resolved_gate:
                        if hasattr(resolved_gate, "request_background_token"):
                            token = await resolved_gate.request_background_token(f"research:{target_domain}")
                        else:
                    ...
                    if resolved_engine:
                        ...
                        logger.warning("CapabilityEngine unavailable; background exploration skipped.")
                        
            except Exception as e:
                record_degradation("curiosity_daemon", e)
