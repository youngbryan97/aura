...
from core.brain.living_mind_context import (
    PRIORITY_GATING,
    LivingMindContext,
    estimate_context_tokens,  # noqa: F401  (read at call time by the lifted module)
)
...
from core.brain.llm.model_registry import (
    ...
    FALLBACK_ENDPOINT,
    PRIMARY_ENDPOINT,
    get_active_cortex_serving_limits,  # noqa: F401  (read at call time by the lifted module)
    lane_display_label,
)
...
from core.conversation.response_reliability import (
    ...
    conversation_reliability_system_block,  # noqa: F401  (read at call time by the lifted module)
...
from core.conversation.user_surface_contract import (
    ...
    resolve_user_surface_prompt,  # noqa: F401  (read at call time by the lifted module)
)
...
from .inference_gate_present_moment import (  # noqa: F401  (re-exported: they were defined here)
    _attach_the_present_moment,
    _modulate_sampling_from_the_body,
    _refresh_volatile_grounding,
)
from .inference_gate_turn_serving import _ServesTheTurn
from .inference_gate_turn_setup import _SetsTheTurnUp
...
from .inference_gate_living_context import _BuildsTheLivingContext
        ...
        for client_path, client in eligible:
            try:
                ...
                if client_path in configured_ladder_paths:
                    shed_ladder_count += 1
                # The next background turn would spawn the worker straight
                # back into the memory that just shed it (LIVE 2026-09-20:
                # the brainstem loaded and was shed 176 times in one uptime,
                # twenty seconds apart). The client's own admission backoff
                # holds a background respawn, and grows while the sheds repeat.
                note_denial = getattr(client, "_note_model_load_admission_denial", None)
                if callable(note_denial):
                    note_denial("memory_pressure_shed", receipt_id=str(reason))
                logger.warning(
