...
from core.brain.llm.latent_cortex.output_quality import (
    evaluate_latent_output,  # noqa: F401  (read at call time by the lifted module)
)
...
class LatentCortexService(_ReasonsDeeply, _ChecksTheReceiptEvidence, _ChecksTheReceiptContract):
    ...
    async def deep_reason(
        ...
        *,
        ...
        self._deep_reason_part_1(config, controller_decision, domain, messages, question, stakes, uncertainty)
        if allocation_profile == "resident_32b_interactive_full_stack_v2":
            try:
                self._deep_reason_build_critic_source_identity(config, controller_decision, domain, messages, question, stakes, uncertainty, worker_identity)
            except (
                ImportError,
                AttributeError,
                OSError,
        ...
        if allocation_profile == "resident_32b_interactive_full_stack_v2":
            ...
            if compound_objective:
                ...
                try:
                    required_wall_clock_s, target_decode_tokens = self._deep_reason_recommended_completion_tokens(capacity_decode_tokens, messages, question, target_decode_tokens)
        ...
        if action_policy_evidence is not None:
            try:
                ...
                policy_receipt, raw_trace = self._deep_reason_policy_receipt(action_policy_evidence, result_receipt)
                ...
                action_policy_matches = self._deep_reason_part_5(action_policy_evidence, action_transitions, external_execution_offer, policy_receipt, raw_trace, result_receipt, validated_trace)
            ...
            if contract_errors == ["answer_replacement_unproven"]:
                ...
                if "vanilla_incumbent_captured_before_adaptation" in flags:
                    try:
                        ...
                        result["receipt"] = result_receipt
            quality_receipt = self._deep_reason_part_6(adaptive_plan, budget, config, contract_errors, quality_receipt, result, result_receipt, visible_objective)
        if operation_lease is not None:
            from core.brain.llm.latent_cortex.epistemic_state import (
                OperationOutcome,
            )
            ...
            if result.get("ok") is True and not action_policy_matches:
                ...
                self._last_failure_receipt = result_receipt
                return failed
        self._deep_reason_part_7(action_policy_evidence, action_policy_matches, epistemic_state, result, result_receipt, selective_memory_result)
        if result.get("ok"):
            if contract_errors:
                failed = self._deep_reason_reason(contract_errors, host_incumbent, result, result_receipt)
                return failed
            ...
            self._deep_reason_part_9(action_transitions, budget, controller_decision, result, result_receipt, started)
            ...
            self._deep_reason_part_10(elapsed, result, result_receipt)
        return result
