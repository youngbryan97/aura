...
from .mlx_client_recording import _RecordsWhatTheWorkerDid
...
from core.brain.llm.measured_admission import (
    record_generation,  # noqa: F401  (re-exported: the lifted recorder reads it here)
)
...
from core.utils.memory_monitor import (
    get_memory_pressure_snapshot,  # noqa: F401  (read at call time by the lifted module)
)
...
from .mlx_client_worker_lifecycle import _KeepsTheWorkerAlive
...
#: Reasons this client records when a request was DECLINED before the worker
#: was ever asked anything. Everything that scores lane health has to be able
#: to tell one of these from a worker that was asked and produced nothing,
#: because the two call for opposite responses: stand down and retry, or mark
#: the lane recovering.
_DECLINED_BEFORE_THE_WORKER = ("skipped_during_foreground_ownership",)
_DECLINED_BEFORE_THE_WORKER_PREFIXES = ("stopped_before_worker_spawn:",)


def was_declined_before_the_worker(reason: str) -> bool:
    """Whether this deliberate-no-text reason means nothing was ever asked."""
    text = str(reason or "")
    return text in _DECLINED_BEFORE_THE_WORKER or text.startswith(
        _DECLINED_BEFORE_THE_WORKER_PREFIXES
    )
...
async def _model_load_admission_context(
    ...
    *,
    ...
    try:
        ...
        # The lease lives as long as the LOAD, not as long as a guess about
        # how long a load takes. The whole load runs inside this context
        # manager on this task, and the controller already reads a holder
        # task that way: a lease with one is held until the task ends,
        # whatever the TTL says.
        #
        # LIVE, 2026-09-21: "Model-load admission lease expired before
        # release lane=cortex" on a contended host — admission believed the
        # cortex's 20GB was free while it was still being read in, and the
        # release that followed found nothing to release. The TTL stays as
        # the backstop for a task that disappears without releasing.
...
class MLXLocalClient(_RecordsWhatTheWorkerDid, _WaitsForTheResult, _KeepsTheWorkerAlive, _KnowsWhichWorkerItIsTalkingTo, _WarmsUpAndSwapsAdapters, _ReasonsInLatentSpace, _RunsTheUnifiedRecurrentLane):
    """
    ...
    async def prove_visible_readiness(
        ...
        *,
        ...
        if not said or not str(said).strip():
            # "No text" is a claim about the worker. When the generation was
            # DECLINED rather than run, saying it is how a lane that was never
            # asked gets marked recovering.
            declined = self.consume_deliberate_no_text_reason()
            if was_declined_before_the_worker(declined):
                return f"declined:{declined}"
            return declined or "no_text"
    ...
    def generation_in_flight(self) -> bool:
        """Whether this worker is producing an answer right now.

        LIVE, 2026-09-21 boot: the brainstem loaded to serve a foreground
        turn, and four seconds later the memory-pressure shed rebooted it —
        "Generation cancelled for Ternary-Bonsai-2-27B-mlx-2bit during
        expected reboot". The turn ended "My response was cut short". The
        shed asked which LANE a worker was, never whether it was busy.

        Shedding a worker mid-generation does not even free the memory
        sooner: the weights are resident either way until the reboot
        finishes, and the answer is gone.

        The same four signals the force-abort path reads, because they are
        the same question.
        """
        current = self._current_gen_future
        ...
        if any(
            ...
            return True
        return self._active_generations > 0 or self._current_request_started_at > 0.0
    ...
    async def _generate_inner(
        ...
        **kwargs,
        ...
        self._deliberate_no_text_reason = None
        if request_is_background and _foreground_owner_active():
            # The primary lane's own warmup is exempt, and the reason is
            # already written at the warmup gate in
            # ``mlx_warmup_and_adapters``: the foreground owner is usually a
            # turn WAITING on exactly this warmup. That exemption was written
            # there and not here, so the gate let the warmup start and this
            # line refused every generation the warmup is made of.
            #
            # LIVE, 2026-09-21, four lines apart: "Skipping queued background
            # generation" for the precompile, "Verifying conversation
            # readiness with a visible probe", "Skipping queued background
            # generation" again, then warmup_readiness_no_text. The lane sat
            # in ``warming`` and the request that needed it waited out its
            # budget — the same deadlock the 2026-07-10 exemption was written
            # to end, arriving through the other door.
            warming_the_primary_lane = bool(
                (kwargs.get("warmup_precompile") or kwargs.get("health_probe"))
                and self._is_primary_lane()
            )
            if not warming_the_primary_lane:
                # A refusal is not an empty answer. Everything that scores this
                # lane reads None as "the worker produced nothing", so the skip
                # has to say it was a skip.
                self._deliberate_no_text_reason = "skipped_during_foreground_ownership"
                logger.info(
            ...
            if background_deferral:
                if not self._can_run_resident_background_health_probe(
                    ...
                    self._deliberate_no_text_reason = (
                        f"stopped_before_worker_spawn:{background_deferral}"
                    )
                    logger.info(
