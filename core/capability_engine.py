    ...
    *netloc is not a hostname.* It carries userinfo and port, so
    ...
    def _is_allowed(self, url: str) -> bool:
        ...
        try:
            ...
            # not a failure: text that will not parse as a URL is not an
            # allowed domain, and this gate only ever withholds.
            return False
...
def _conversion_penalty() -> int:
    ...
    try:
        ...
        # not a failure: with no ledger to read, nothing has been measured
        # to penalise, and zero leaves the ceiling where it was.
        return 0
...
def _never_taught(skill_name: str) -> bool:
    ...
    try:
        ...
        # not a failure: with no ledger, nothing is known to be untaught,
        # and False withholds the repair rather than offering it wrongly.
        return False
...
class CapabilityEngine(_AsksWhetherThePersonWouldWantThis, AuraBaseModule):
    ...
    def _load_default_trigger_patterns(self) -> None:
        ...
        # The table itself lives in core/skills/default_trigger_patterns.py:
        # 415 lines of literal vocabulary inside one method of this module,
        # changing for different reasons and at a different rate from the
        # routing that reads it.
        from core.skills.default_trigger_patterns import default_trigger_patterns

        patterns = default_trigger_patterns()
    ...
    def _is_path_within_workspace(path: Any) -> bool:
        ...
        try:
            ...
            # not a failure: a path that will not resolve cannot be shown to
            # be inside the root, and this gate only ever withholds.
            return False
    ...
    def _computer_use_effect_scope(params: dict[str, Any]) -> str | None:
        ...
        if action == "run_command":
            try:
                ...
                # not a failure: a command line that will not lex names no
                # capability, and None is how the caller hears that.
                return None
            if not argv:
                return None
    ...
    def capability_maturity(self, skill_name: str) -> Any:
        ...
        try:
            ...
            # not a failure: a build without the maturity grader returns no
            # grade, and every caller checks for None.
            return None
    ...
    def _looks_like_search_capability_question(text: str) -> bool:
        ...
        try:
            ...
            # not a failure: the comment above says a resolving filename SETTLES the routing.
            named = None
    ...
    def _apply_executive_constraints(ctx: dict[str, Any]) -> dict[str, Any]:
        ...
        if timeout_raw is not None:
            try:
                ...
                # not a failure: a timeout that is not a number is no
                # timeout, and the line below only acts on a positive one.
                timeout_s = 0.0
            if timeout_s > 0.0:
        ...
        if outcome == "deferred":
            ...
            if retry_after is not None:
                try:
                    ...
                    # not a failure: a retry hint that is not a number is no
                    # hint, and the payload carries the deferral either way.
                    pass
            return payload
    ...
    async def execute(
        ...
        async def _execute_wrapped() -> dict[str, Any]:
            ...
            try:
                ...
                # not a failure: a constraint that is not a number is no
                # constraint, and the line below only acts on a positive one.
            ...
            try:
                ...
                try:
                    ...
                    # The trace is what dev mode SHOWS of a tool call.
                    logger.debug("Tool execution was not traced: %s", exc)
                    tool_trace = None
...
def live_capability_engine() -> "CapabilityEngine | None":
    ...
    try:
        ...
        # not a failure: the docstring says asking the container first finds
        # the warm one, and not finding it is the answer to that.
        return None
