...
from .response_reliability_requests import (  # noqa: F401  (re-exported: they were defined here)
    _BREVITY_PSEUDO_PHRASE_HEADS,
    _INCLUDE_REQUIRED_PHRASE_RE,
    _QUOTED_REQUIRED_PHRASE_RE,
    _USE_REQUIRED_PHRASE_RE,
    _compact_output_style_requested,
    _compact_reference_acknowledgement,
    _constraint_match_is_actionable,
    _count_token_to_int,
    _is_escaped_character,
    ...
    _text_index_is_unquoted,
    _word_count,
    _word_count_token_to_int,
...
from .response_reliability_sentences import (  # noqa: F401  (re-exported: they were defined here)
    _default_followup_question,
    _expand_sentence_candidates,
    _finish_sentence_fragment,
    _listify_sentences,
    _number_sentences,
    _pad_sentence_candidates,
    _paragraphize_sentences,
    _split_long_sentence_once,
    _split_sentences,
)
...
def _has_empty_requested_list_item(reply_text: Any, count: int) -> bool:
    if count <= 1:
        return False
    bodies = _list_item_bodies(reply_text)
    if not bodies:
        return False
    return any(_word_count(body) == 0 for body in bodies[:count])
...
def _compound_request_coverage_reasons(
    ...
    missed = unanswered_question_parts(reply, shape)
    if not missed:
        return []
    # Say which part, and what the gate took as the question: a verdict
    # with only its name was undiagnosable from the feed (LIVE 2026-09-19,
    # a right answer to a word problem, twice).
    logger.info(
        "coverage: %d of %d part(s) unanswered — missed %r; question read as %r; reply head %r",
        len(missed),
        len(getattr(shape, "question_segments", ()) or ()),
        [str(part)[:80] for part in missed[:3]],
        user[:160],
        reply[:80],
    )
    return ["unanswered_question_part"]
...
def _has_unsupported_runtime_limits_claim(user_message: Any, reply_text: Any) -> bool:
    ...
    if not raw or not _UNSUPPORTED_RUNTIME_LIMITS_CLAIM_RE.search(raw):
        return False
    prompt = _normalize(user_message)
    asks_actual_capability = names_any(prompt, (
            "could you actually",
            "can you actually",
            "try it",
            "open my",
...
def grounded_operational_status_reply(user_message: Any, reply_text: Any = "") -> str:
    ...
    mentions_tools = names_any(lower, (
            "tool",
            "tools",
            "desktop",
            "os control",
...
def _has_self_reflection_substance(reply_text: Any) -> bool:
    ...
    if _reply_has_pseudo_internal_jargon(reply_text):
        return False
    concrete_attention = names_any(reply, (
            "attention",
            "focus",
            "noticing",
            "feel",
...
def _assess_user_facing_reply(
    ...
    *,
    ...
    if is_confusion_repair_turn(user_message) and _LOW_SIGNAL_REASSURANCE_RE.match(raw):
        if not (_word_count(raw) >= 3 and names_any(raw.lower(), ("thinking", "working", "processing", "online"))):
...
from .response_repetition import (  # noqa: E402,F401  (re-exported for callers)
    _distinct_statement_ratio,
    _phrase_loop_reason,
    _topical_stem,
