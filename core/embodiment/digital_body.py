...
class DigitalBody:
    ...
    def register_commitment(self, commitment: Dict[str, Any]) -> None:
        """Registers an active commitment (goal) with metadata."""
        with self._lock:
            ...
            logger.info("DigitalBody registered commitment: %s (%s)", commitment.get("id"), commitment.get("goal"))

    def resolve_commitment(self, commitment_id: str, status: str = "completed") -> Optional[Dict[str, Any]]:
        """Resolves an active commitment and returns it."""
        with self._lock:
            for commitment in self.current_commitments:
                if commitment.get("id") == commitment_id:
                    commitment["status"] = status
                    commitment["resolved_at"] = time.time()
                    logger.info("DigitalBody commitment %s resolved as: %s", commitment_id, status)
                    return commitment
            return None
