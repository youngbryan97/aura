...
def audit_semantic_cohort(model, examples, *, directory, max_charts=16,
    ...
    for item in examples:
        ...
        if path.exists():
            ...
            observation = _observe(model, item)
            diagnosis = None
            if observation['semantic_status'] == 'equivalent':
                ...
                bank = decode_semantic_candidates(model,
                    ...
                    model_basis_sha256=item.ir.model_basis_receipt_sha256, **options)
                diagnosis = diagnose_semantic_candidate_bank(bank, item)
                stage = diagnosis['failure_stage']
            body = {'schema': 'aura.semantic_cohort_diagnosis_row.v1', 'identity': identity,
    ...
    body = {'schema': 'aura.semantic_cohort_diagnosis.v1', 'identity': identity,
