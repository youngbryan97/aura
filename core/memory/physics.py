...
def bekenstein_check(data_bits: int, radius_cm: float, energy_mj: float) -> dict[str, float | int | bool]:
...
def hawking_decay(stored_at_ms: int, key_string: str) -> dict[str, float]:
    """
...
def grav_queue_sort(items: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """
    ...
    def _score(item: dict[str, Any]) -> float:
        ...
        age = (time.time() * 1000) - float(item.get("created", 0))
        return float(item.get('access_count', 0)) / max(1.0, age)
