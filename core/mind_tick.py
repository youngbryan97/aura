from .mind_tick_loop_steps import _RunsTheTickLoopSteps
import asyncio
import hashlib
import logging
import os
...
from core.health.degraded_events import record_degraded_event  # noqa: F401  (read at call time by the lifted module)
...
from core.runtime.progress_bound import run_on_a_thread_while_it_works  # noqa: F401  (read at call time by the lifted module)
from core.runtime.service_access import resolve_inference_gate  # noqa: F401  (read at call time by the lifted module)
...
class MindTick(_RunsTheTickLoopSteps, _KnowsWhetherItIsStillAlive):
    """
        ...
        while self._running:
            ...
            try:
                ...
                if not state:
                    ...
                    continue
                await self._run_loop_health_pause()
                ...
                audit_interval = await self._run_loop_bridge_event_bus(metadata, start_time)
                ...
                local_runtime_state = self._run_loop_check_sidecar_process()
                    ...
                    if idle_time > 1200.0 and len(current_state.cognition.working_memory) >= 5:
                        ...
                        if memory_coord and hasattr(memory_coord, "consolidate_working_memory"):
                            ...
                            if not decision or not decision.is_approved():
                                ...
                                await self._run_loop_these_run_during()
                ...
                elapsed, interval = self._run_loop_always_advance_heartbeat(sleep_time_override, start_time)
                if elapsed > 5.0:
