...
def _wall_clock_allowance(timeout_s: float) -> float:
    """What the runner will wait, so the dispatcher waits at least as long."""

    from core.sandbox.runner import wall_clock_allowance

    return wall_clock_allowance(timeout_s)
...
class CodeREPLSkill(BaseSkill):
    ...
    #: turn.
    timeout_seconds = _wall_clock_allowance(_LONGEST_ACCEPTED_TIMEOUT_S)
    metabolic_cost = 2
