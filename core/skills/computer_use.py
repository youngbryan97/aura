...
import sys
import time
import urllib.parse  # noqa: F401  (re-exported: the lifted browser reads it here)
import webbrowser
...
from core.conversation.word_markers import (
    names_any as names_any,  # re-exported: the lifted browser reads it here
)
...
from .computer_use_browser import _ReadsAndDrivesTheBrowser
...
def _quartz_error_types() -> tuple[type[BaseException], ...]:
    ...
    try:
        ...
        pass  # not a failure: no PyObjC off macOS, so no objc.error
    return tuple(errors)
...
class ComputerUseSkill(_ReadsAndDrivesTheBrowser, _MakesFilesToShow, BaseSkill):
    ...
    async def _write_through_dictionary(
        ...
        try:
            ...
            observed_chars = 0  # not a failure: no count means nothing observed
        observed_title = (
...
""".strip()
    ...
    def _runtime_permission_payload(message: str) -> dict[str, Any] | None:
        try:
            ...
            return None  # not a failure: no guard, no permission

        try:
            guard = get_permission_guard()
        except _COMPUTER_USE_RECOVERABLE_ERRORS:
            return None  # not a failure: a silent guard names no permission
    ...
    def _allowed_readable_roots(self) -> list[Path]:
        ...
        try:
            ...
            pass  # not a failure: the three artifact roots are already listed
        return roots
    ...
    def _safe_directory_walk(self, start_dir: str, max_depth: int = 4, max_files: int = 250) -> str:
        ...
        def walk_dir(current_path: Path, prefix: str, depth: int):
            ...
            try:
                ...
                # not a failure: a directory she may not read is a fact
