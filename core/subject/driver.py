        ...
        for phase in self.kernel._phases:
            ...
            try:
                result = await within_budget(
                    ...
                    phase_budget(self, name, condition.origin, fallback=PHASE_TIMEOUT),
                )
...
def build_runtime(
    ...
    install_mind(kernel, mind or DeterministicMind())
...
async def start_organism(runtime: SubjectRuntime, *, quiet: bool = False) -> dict[str, Any]:
    ...
    if runtime.whole:
        summary["language"] = await bring_up_language(runtime)
