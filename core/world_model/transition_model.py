...
_transition_model_instance: TransitionModel | None = None
...
class TransitionModel:
    ...
    def __init__(self, learning_rate: float = 0.05) -> None:
        ...
        self.last_action_vec: np.ndarray | None = None
        ...
        with self._lock:
            ...
            if self.last_state is not None and self.last_action_vec is not None:
                ...
                self._record_to_ledger(
                    self.last_state, self.last_action_name, s_hat, current_state, e_t
                )
    ...
    def _record_to_ledger(
        ...
        try:
            ...
            record_degradation(
                "transition_ledger_log",
                exc,
                severity="warning",
                action="skipped cognitive ledger append after transition receipt construction failed",
            )
    ...
    def get_state_dict(self) -> dict[str, Any]:
        ...
        with self._lock:
            mean_error = (
                float(np.mean(self.prediction_error_history))
