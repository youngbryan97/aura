...
- [ ] G03 Close learned semantic binding/composition failures on development
  ...
  held-out source-bank measurement and promotion remain open.
  ...
  changed cases are confined to two arithmetic constructions. No broad
  transfer or serving claim follows; G03 remains open.
  [Full-fold triadic addendum](evidence/G03_TRIADIC_FULL_FOLD_ADDENDUM_2026-09-24.md):
  the frozen fold-1 candidate falls from 214/254 to 210/254 exact programs
  despite its 40-source pilot gain; it is rejected, and graph-level factor
  training remains open. Tokenizer-compiled grounding matches its inherited
  contract, but this does not establish end-to-end unseen-input transfer.
  ...
  [Full-portfolio gap attribution](evidence/G03_PORTFOLIO_GAP_ATTRIBUTION_2026-09-22.md)
  separates missing candidates from wrong selection on the 500 exposed tasks.
...
- [ ] Q09 Resolve order-dependent tests; no isolated pass erases a batch fail.
  ...
  in the test, entered in docs/ORDER_DEPENDENCE_REGISTER.md. The habit arbiter
  test was the same disease: a worker that had run organism turns left her
  tired, and the fatigue filter set aside the initiative the habit discount put
  last. A test that tires her first reproduces it; the fixture now empties every
  ledger arbitration reads. Still open: the ghost chain's writer thread
  reopening `~/.aura-test/data/ghost/chain` after the teardown sweep, a path
  that does not follow the run's state root.
