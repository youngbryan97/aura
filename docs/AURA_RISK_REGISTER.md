...
| R-003 | Sync execution blocks event loop | Critical | Medium | Legacy infrastructure BaseSkill offloads sync `execute()` via `asyncio.to_thread`; lockdep reports every hold over 50ms on the loop, and five of the nine seen in one boot on 2026-09-08 are fixed — atomspace, both ontogeny singletons, and the morphogenesis graph, registry and field. All five were one shape: the lock held across the WORK rather than across the read the work needs | Four holds remain measured and open: `core.runtime.health_fragments` 273ms, `core.language.learned_matcher` 148ms, `earned_metric.axis.valence` 87ms, `core.canonical.state.singleton` 53ms |
...
| R-006 | `StateVaultActor` readiness non-fatal in strict runtime | Critical | Low | Strict runtime now aborts boot on failed vault handshake; regression added | Extend strict fail-closed behavior to other critical services |
...
| R-032 | The resident model's prompt cache can never be trimmed | High | High | `Qwen3_5.make_cache` returns `ArraysCache` for its linear-attention layers and `ArraysCache.is_trimmable` is a bare `return False`, so `can_trim_prompt_cache` is False for every prompt cache this model makes. The exact-hit and trimmed-hit reuse paths are therefore dead, permanently, and only a stored prompt that is a strict PREFIX of the new one can be reused. Measured live 2026-09-08: a five-step tool loop matching 17%, 35%, 47% then 55% of its own prompt and prefilling all of it each time, 40s to 83s to first token, and the turn's 178.8-second budget gone. The refusal now names the class (`refuses to trim (ArraysCache)`) instead of reading as an absence, and the per-turn prompt block moved to the end of a tool loop so each step extends the one before it | Measure the reuse the append-only prompt actually buys, then decide whether an untrimmable-cache model needs a different retention strategy |
