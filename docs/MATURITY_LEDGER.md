...
| Wave3 #12 | P1 | Her library offers no leaves, so the search horizon has never moved | Traced: the chain is whole. `keep` writes heads, `recall` restores them, the conductor calls recall at start and keeps periodically, and the live file at ~/.aura/meanings_she_induced.json holds one induced kind and `"heads": {}`. Nothing has ever written a head, so the horizon has never moved for want of input rather than for want of machinery. The open question is why `the_head_she_wrote` has never fired live. | TODO |
