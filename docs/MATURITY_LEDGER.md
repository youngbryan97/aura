...
| Wave3 #10 | P1 | Two of three matched arms still cannot be measured | Sized: of ten declared lesion channels, one is applied where a direct model call reaches it and ten are applied where a turn through the engine does. Routing the arms through the engine takes measurable faculties from one to ten. | TODO |
...
| Wave3 #12 | P1 | Her library offers no leaves, so the search horizon has never moved | Traced by running it. Every step fires: three pairs of her parts share something, the action writes an 87-symbol head, the gate measures it on the held-out families, it pays on none, and it is put back. Nothing is starved of candidates. The live record holds 512 episodes across 76 families of which THREE carry their cases, and three binary observations cannot lift a posterior over the threshold — so the gate refuses a change that helps and one that does nothing alike. Growing the probe is upstream of everything. | TODO |
