...
| Wave2 #5 | P1 | A checkpoint can become durable before the writes that produced it | The draining primitive exists and is imported by exactly one module, the two-provider checkpoint store — and nothing imports that store. The census now calls that what it is: governing a proposal. The checkpointer that actually runs is durable_workflow, which saves after every step and drains nothing, because no write registers itself as in flight. Wiring the wait alone would give a gate with nothing ever registered, which is a mechanism that cannot fire; the registration belongs at the file write gateway's async methods, scoped to the work that launched them, and that is the change still owed. | TODO |
| Wave2 #6 | P1 | Checkpoints have no branch or parent lineage | The checkpointer that runs has most of it: durable_workflow's checkpoint carries a revision bumped on every save, a history directory holding every revision, load by revision, and forked_from with forked_at_revision. What it did not have was prune, and every step writes a revision, so a workflow's history grew for as long as the workflow ran. WorkflowStore.prune keeps the newest revisions the caller asks for — no default, how much history is worth holding is not a number to pick here — and never removes a revision another workflow was forked from, because a branch whose parent file is gone has lost the lineage the history exists to keep. Deletion goes through the write gateway under an internal governance scope; raw file mutations stay at 843. The two-provider store written for lineage is a second checkpointer nothing imports, left for folding in or retiring. | DONE |
...
| Wave2 #9 | P1 | 28 of 162 organs say nothing about how failure propagates | Fifteen of 165 on 2026-09-14, and the number hid what mattered. Those fifteen held 117 exception handlers; ten of them did nothing but pass or continue. Three were control flow and stay — a path not under this root, a control arm that cannot be derived, an attribute probe. Seven were failures that vanished: a configured workspace root that would not resolve, so every write under it was refused with no reason; two container registrations, so a lookup found nothing; malformed rows in the battery's edge file, dropped from a corroboration; a source file the layer scan could not parse, so its channels were absent from every layer; an entropy source that failed, so a collapse meant to use real entropy became reproducible; and a research ledger that stopped resolving predictions. Each is recorded, logged with its count, or counted where a caller reads it. Thirteen organs are still on the list, and two of those are on it by the proxy rather than the defect: the count asks for `record_degradation(` in the source, and core/quantum imports nothing from the runtime, so its fallback is counted on the instance instead and the scan cannot see that. | TODO |
...
| Wave2 #11 | P0 | Generated Python was delivered successfully and was semantically wrong | Queued job coroutines never awaited; a deadlock waiting on queue data while holding a producer lock; direct deque mutation bypassing put(); four never-awaited coroutine warnings; a false claim that cancellation releases an asyncio.Lock. The checker for every one of those runs on every generation, and until now that was all it did: the findings went to a log line and to a field nothing read, and the code that dropped its coroutines was still the code that shipped. A draft with mistakes is redrafted once with the checker's own findings attached as data, the way the skill forge hands a drafter its traceback. The draft with fewer mistakes is served — the first on a tie, because nothing says the second is better when the checker cannot tell them apart — and a second attempt that is deferred, empty or unparseable keeps the first. What was found stays recorded against the draft that shipped. | DONE |
| Wave2 #12 | P1 | Cancellation is not linked to the future representing the call | Two mechanisms had been built for it and they did not compose. `what_stops_it` is the token that governs — seven production modules read it, the inference gate among them. `cancelling_the_call_and_not_just_the_wait` tied a give-up to the awaited future with a threading.Event of its own, so a caller that gave up reached the handler it had handed the call to and stopped there: the generation that handler was awaiting read the ambient token, saw nothing, and held the model lane to the end of a turn nobody was waiting for. The work now runs under a child of the ambient token and giving up stops that token. A test that only the ambient token can satisfy fails on the old module and passes on this one, and a child giving up still cannot stop its parent. | DONE |
| Wave2 #13 | P1 | A prepared action batch is mutable and unordered | The batch primitive existed, was tested, and governed nothing: only the inspector imported it. The production plan runner, ToolExecutor.execute_plan, was a loop over a mutable list that appended results as it went and recorded a call that was not a call the same way as a tool that ran. It runs through the primitive now — the plan frozen with an id on every action before anything runs, a non-call BLOCKED rather than FAILED, results in the order decided, and every plan in the batch ledger. One at a time, as before: tools in a plan share a filesystem, and making them concurrent is a behaviour change this adoption does not make. | DONE |
...
| Wave3 #10 | P1 | Two of three matched arms still cannot be measured | Sized first: ten channels declared, one reachable by a direct model call, ten by a turn through the engine. Then asked what each hands over, which is the half the first reading missed — six of ten hand over a sampler parameter or prompt text, and a harness standing in the right place still cannot measure a value it has no way to consume. Three of those six were declared, lesionable and EMPTY here, because nothing in the harness had produced the frames they gate. The three advisory passes are plain functions over an AuraState and a prompt, so they run without a runtime; with the frames made, the endogenous arm removes four channels instead of one. Run 4: intact samples at 0.4968 with three biases carrying, the arm samples at 0.5000 with none, delta 0.0417, intervals overlapping — an honest null with two independent readings behind it. The remaining four need a decoder that loops or a consumer downstream of the reply, which is a fact about them and not about this harness. | DONE |
| Wave3 #11 | P1 | The skill forge is not a closed loop | Every arrow exists and each is reported somewhere; what was not reported is which one is empty, and on a loop paying nothing that is the only thing worth knowing. Seven steps counted in order — gaps seen, gaps worth answering, candidates drafted, verified, installed, taken, gaps that stopped — and the reading is the FIRST step with nothing in it. Steps after it never had input and calling them failures is how one broken link reads as five. A forge with forty gaps and nothing ever taken and a forge that has seen nothing both used to read as closed_the_gap: 0. The target the candidate is judged against is held still now (#18) and travels beside the chain, because a loop that turns over on a target the drafter kept moving is not the same reading as one that turns over on a target that held. | DONE |
| Wave3 #12 | P1 | Her library offers no leaves, so the search horizon has never moved | Traced by running it, and the trace was right about the mechanism and wrong about the cause. Every step fires. What starves the gate is the record: two kinds of episode share one 512-entry ring, and only one kind carries the cases a change is judged on. The other is a degradation from some subsystem, which has no cases and never will, and every degradation in the process writes one. Trimming dropped the oldest, so the noisiest part of the system decided what the developmental gate got to weigh. Read off the live record on 2026-09-09: 512 episodes, 107 families, NOT ONE carrying its cases. The gate refused a change that helps and one that does nothing alike. The case-less go first now; ten survive a flood of seven hundred where none did. | DONE |
| Wave3 #13 | P1 | The campaign's admission bar may be unmeetable on this host | Asked on 2026-09-09 and the record was empty, considered 0. Not a bar refusing: the runtime has not been up since the counting was added, so nothing has reached the job to be refused. The reading is BLOCKED on a live stretch rather than open, and the difference matters — an empty record with no runtime says nothing about the bar, and reading it as 'never admitted' would be the same mistake as reading a lesion of an empty channel as a null. | BLOCKED |
...
| Wave3 #15 | P1 | The size ratchet had started refusing the work it exists to cause | Its decomposition test asked whether the tree was under budget AFTER a simulated split, so once the tree drifted past the budget the answer was no however much the split helped — the exact failure its own docstring warns about one level down. Asked differentially now: no refusal the tree did not already have, and the one number the ratchet holds goes down. The debt itself is not laundered and stays red at 178,382 against 145,896. | DONE |
| Wave3 #16 | P0 | 178,382 lines of module oversize against a budget of 145,896, and 38 files over their own baseline | The one number the ratchet holds, growing under several hands. The tractable half is the thirteen god objects created SINCE the baseline, which are never grandfathered. Four are closed: incoming_logic lost the three helpers that already took the turn rather than self; model_registry lost where the artifacts live and the four shapes it hands around; InteriorityService and MemoryConsolidator lost nine functions that had been filed under a class. Nine left, and none of them reaches the ceiling by lifting pure functions alone — phantom_browser has one, black_hole_vault three, morphogenesis/runtime six, against ceilings that need two, five and nine. Those need a responsibility split rather than a lift. The scan that found them was wrong twice before it was right: an ast.Attribute walk cannot see `getattr(self, "principal", "")`, so it called a method pure that reads the instance, and lifting it left an undefined name. Count every mention of the parameter, not every attribute on it. | TODO |
| Wave3 #17 | P2 | The census asks what governs of a curated list of primitives | Asked of every module under the production roots, and asked wrong the first time. Imported by production code was the whole test, so a module whose only importers were themselves unreached read as governing. Carried to a fixed point on 2026-09-14: of 3,645 modules, 2,231 govern, 430 govern only something nothing reaches, 198 are reachable from a tool, 129 are reached by name and 657 are proposals. The share that governs is 61%, not the 74% the first census reported. A cycle with no member reached from outside still reads as governing; telling that apart needs a root set the census does not have yet. | DONE |
| Wave3 #18 | P1 | The forge's redraft returned new probes as well as new code | A drafter that could not satisfy an expectation could rewrite it, and the attempt that passed was scored against a question the failing attempt never faced — while every count downstream still read 'verified'. The first usable probe set fixes the target; a later attempt may add probes and may not change or drop one, and what it tried to do is counted. | DONE |
