...
- **One transducer now learns more than one program shape.** Tokenizer-bound
  grounding supplies exact typed public inputs, while resident hidden states
  supply learned step count, primitive meaning and references to prior results.
  The router-free fit recovered 258/368 complete held-out programs across three
  families and exact execution emitted 292/368 answers. Hidden-token shuffle
  and coefficient lesion each recovered 0/368 programs. The measured source,
  coefficient receipt and 1,296-example feature basis are bound into an
  independently replayed certificate. The supported primitive and type
  vocabulary remains declared; a new schema is not yet learned from scratch.
- **The learned program and endogenous language now meet below Python.** The
  complete typed primitive vocabulary compiles to closed terms on the universal
  metered floor. A no-refit replay found 368/368 identical direct-versus-floor
  outcomes: 366 values and two typed refusals across all three test families.
