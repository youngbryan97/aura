...
| 11 | Epistemic independence: no important adaptive mechanism defines its own success criterion after observing its result | **DONE** — `core/verify/epistemic_independence.py` seals a criterion before the run and refuses a redefinition after it has judged; `support_for` requires two channels the mechanism does not control, and any independent disagreement withholds support; `make epistemic-independence` finds bars derived from what they judge, checked against its null |
...
| 16 | Architectural self-discovery: shadow architectures, forked trials, compatibility interfaces, migration proofs | **DONE** — `core/promotion/shadow_architecture.py`; the shadow's answer cannot reach the caller by any argument, and migration needs no regression on what already worked rather than a good average |
| 17 | Primitive invention that feeds later invention, rather than composition of human-supplied primitives | **DONE** — `core/cognition/invention_depth.py`; novelty judged against the closure rather than the base set, and depth measures whether anything compounds — a wide vocabulary of generation-one inventions is depth 1 |
...
| 24 | Development at one level alters development at the level below: the task, the policy and the learning mechanism as manipulable objects of the same sort, so "improving how I improve" is a search rather than a new module | **DONE** — `core/learning/manipulable_learning.py`; one search function at both levels, and an AST test that it never reads which level it is at |
| 23 | Native cognitive medium: internal representations optimised for computation rather than communication | **DONE** — `core/cognition/native_concepts.py`; untranslatable is not enough, so RESIDUAL is a reachable verdict — an instrument whose disappointing outcome cannot occur agrees with whoever ran it |
