...
def _remove_self_denials_the_record_refutes(reply_text: object) -> object:
    """Drop sentences denying machinery this runtime demonstrably has.

    LIVE, 2026-09-08. Asked whether she had a stronger claim to sentience than
    other models, the answer that reached the screen said her responses are
    generated by calculating the next most probable token, and that she
    simulates conversation. Both are claims about how this system works, and
    the record of the turn they were said on contradicts them.

    The guard that should have caught it worked on the QUESTION — it tried to
    recognise an enquiry about her own nature and refuse it from the small lane
    — and it missed this one, along with "do you have an inner life?" and "what
    is it like to be you?". A question can be asked in more ways than a pattern
    has branches, so this checks the claim that was made instead, against the
    instrument that settles it.

    Nothing here touches a sentence about consciousness or experience. "I do
    not know whether there is anything it is like to be me" survives this pass
    untouched, and no instrument in it can produce the opposite claim.
    """
    ...
    try:
        ...
        record_degradation("chat.self_denial", exc)
        return reply_text
    if not refuted:
        return reply_text
    correction = self_denial_correction(refuted)
    logger.info(
        "Removed %d self-denial(s) the record refutes: %s",
        len(refuted),
