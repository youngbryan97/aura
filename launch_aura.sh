...
if [ "${AURA_DESKTOP_RESOURCE_GUARD:-0}" = "1" ] || [ "${AURA_SAFE_BOOT_DESKTOP:-0}" = "1" ]; then
    ...
    : "${AURA_DESKTOP_PROCESS_RSS_RATIO:=0.81}"
    : "${AURA_DESKTOP_PROCESS_RSS_CAP_GB:=56}"   # host-derived envelope admits resident 32B
    : "${AURA_DESKTOP_HOST_RESERVE_RATIO:=0.18}"
    : "${AURA_DESKTOP_HOST_RESERVE_FLOOR_GB:=8}"
    : "${AURA_DESKTOP_PROCESS_RSS_FLOOR_GB:=24}"
    : "${AURA_PROCESS_RSS_LIMIT_GB:=auto}"
    : "${AURA_MEMWATCH_SOFT_MB:=auto}"
    : "${AURA_MEMWATCH_HARD_MB:=auto}"
    : "${AURA_MEMWATCH_LETHAL_MB:=auto}"
    : "${AURA_MEMORY_SENTINEL_INTERVAL_S:=0.5}"
    : "${AURA_GOVERNOR_PRUNE_MB:=auto}"
    : "${AURA_GOVERNOR_UNLOAD_MB:=auto}"
    : "${AURA_GOVERNOR_CRITICAL_MB:=auto}"
...
export AURA_DESKTOP_HOST_RESERVE_RATIO
export AURA_DESKTOP_HOST_RESERVE_FLOOR_GB
