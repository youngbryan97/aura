...
def test_flagship_gate_flags_raw_asyncio_task_in_production(tmp_path: Path):
    ...
    get_task_tracker().create_task(get_storage_gateway().create_dir(prod.parent, cause='test_flagship_gate_flags_raw_asyncio_task_in_production'))
...
def test_flagship_gate_allows_tests_to_create_tasks(tmp_path: Path):
    ...
    get_task_tracker().create_task(get_storage_gateway().create_dir(test.parent, cause='test_flagship_gate_allows_tests_to_create_tasks'))
...
def test_flagship_gate_flags_async_sys_exit(tmp_path: Path):
    ...
    get_task_tracker().create_task(get_storage_gateway().create_dir(prod.parent, cause='test_flagship_gate_flags_async_sys_exit'))
...
def test_flagship_gate_flags_import_time_async_primitives(tmp_path: Path):
    ...
    get_task_tracker().create_task(get_storage_gateway().create_dir(prod.parent, cause='test_flagship_gate_flags_import_time_async_primitives'))
    prod.write_text("import asyncio\nLOCK = asyncio.Lock()\n", encoding="utf-8")
    report = scan_codebase(tmp_path)
