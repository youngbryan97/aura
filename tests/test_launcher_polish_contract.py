...
def test_native_bridge_targets_input_effects_and_verifies_activation():
    ...
    assert 'env["AURA_PROCESS_RSS_LIMIT_GB"] = "auto"' in swift
    assert 'env["AURA_MEMWATCH_SOFT_MB"] = "auto"' in swift
    assert 'env["AURA_MEMWATCH_HARD_MB"] = "auto"' in swift
    assert 'env["AURA_MEMWATCH_LETHAL_MB"] = "auto"' in swift
    assert 'env["AURA_MEMORY_SENTINEL_INTERVAL_S"] = "0.5"' in swift
    assert 'env["AURA_GOVERNOR_PRUNE_MB"] = "auto"' in swift
    assert 'env["AURA_GOVERNOR_UNLOAD_MB"] = "auto"' in swift
    assert 'env["AURA_GOVERNOR_CRITICAL_MB"] = "auto"' in swift
...
def test_launch_script_supports_gui_window_mode():
    ...
    assert "AURA_PROCESS_RSS_LIMIT_GB:=auto" in shell
    assert "AURA_MEMWATCH_SOFT_MB:=auto" in shell
    assert "AURA_MEMWATCH_HARD_MB:=auto" in shell
    assert "AURA_MEMWATCH_LETHAL_MB:=auto" in shell
    assert "AURA_MEMORY_SENTINEL_INTERVAL_S:=0.5" in shell
    assert "AURA_GOVERNOR_PRUNE_MB:=auto" in shell
    assert "AURA_GOVERNOR_UNLOAD_MB:=auto" in shell
    assert "AURA_GOVERNOR_CRITICAL_MB:=auto" in shell
