...
def test_native_bridge_targets_input_effects_and_verifies_activation():
    ...
    assert 'env["AURA_MEMWATCH_SOFT_MB"] = "37888"' in swift
    assert 'env["AURA_MEMWATCH_HARD_MB"] = "41984"' in swift
    ...
    assert 'env["AURA_GOVERNOR_CRITICAL_MB"] = "40960"' in swift
...
def test_launch_script_supports_gui_window_mode():
    ...
    assert "AURA_MEMWATCH_SOFT_MB:=37888" in shell
    assert "AURA_MEMWATCH_HARD_MB:=41984" in shell
    ...
    assert "AURA_GOVERNOR_CRITICAL_MB:=40960" in shell
