...
def test_recovery_surfaces_do_not_assert_unmeasured_runtime_facts(monkeypatch):
    ...
    patch_chat_lane(monkeypatch, "_shape_with_live_substrate", lambda text, user_message="": text,
    )
    replies = (
...
async def test_api_chat_projects_verified_answer_provenance_without_quality_repair(
    ...
    chat_common._conversation_log.clear()
    prior_answer = "Stanisław Lem"
    provenance = AnswerProvenance(
        answer_sha256=hashlib.sha256(prior_answer.encode("utf-8")).hexdigest(),
        session_id="answer-provenance-live-route",
        turn_id="prior-answer-turn",
        captured_at=1.0,
        response_path="cognitive_engine",
        model_native_inference=True,
    )
    chat_common._conversation_log.append(
        {
            "id": "prior-answer-turn",
            "user": "Who wrote the novel Solaris?",
            "aura": prior_answer,
            "status": "complete",
            "session_id": "answer-provenance-live-route",
            "metadata": {"answer_provenance": provenance.to_dict()},
        }
    )
...
async def test_rejected_generation_cannot_open_metadata_less_second_owner(monkeypatch):
    ...
    patch_chat_lane(monkeypatch, "_ground_runtime_fact_status_reply", lambda _visible, reply, _lane, **_kwargs: f"{reply} grounded",
    )
    monkeypatch.setattr(
        chat_routes.ServiceContainer,
        "get",
...
def test_grounded_self_condition_reply_forwards_session_identity(monkeypatch):
    ...
    patch_chat_lane(monkeypatch, "_build_self_condition_evidence", _evidence)
