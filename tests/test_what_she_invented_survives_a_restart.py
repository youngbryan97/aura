...
from core.runtime.what_she_invented import forget_everything, keep, recall
...
def _a_clean_mind(tmp_path, monkeypatch):
    from core.runtime import what_she_invented

    monkeypatch.setattr(what_she_invented, "_KEPT_AT", tmp_path / "invented.json")
...
def test_a_recipe_that_cannot_be_read_is_skipped_rather_than_run(tmp_path, monkeypatch):
    ...
    where = tmp_path / "bad.json"
    where.write_text('{"measures": [{"at": "nowhere"}], "meanings": {"x": {}}}')
    monkeypatch.setattr(what_she_invented, "_KEPT_AT", where)
    assert recall() == {"measures": 0, "meanings": 0}
