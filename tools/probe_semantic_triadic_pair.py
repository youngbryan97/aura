...
def main() -> None:
    ...
    parser.add_argument("--offset", type=int, default=0,
                        help="skip this many source-hash-ordered held identities")
    parser.add_argument("--output", type=Path, required=True)
    args = parser.parse_args()
    if args.offset < 0 or args.limit < 0 or (args.offset and not args.limit):
        parser.error("paired held slice requires nonnegative offset and a positive limit")
    ...
    if "graph_calibration_cohort" in report:
        ...
        cohort = report["graph_calibration_cohort"]
        graph_calibration, expected_cohort = select_triadic_score_calibration_sources(
            graph_population, limit=(cohort["selected_sources"]
                                     if cohort["selected_sources"] != len(graph_population) else 0))
        if cohort != expected_cohort:
            raise ValueError("paired triadic graph calibration cohort changed")
    else:
        graph_calibration = tuple(calibration)
    ...
    parent_cohort_matches = (
        parent.training_receipt.get("training_example_ids_sha256")
        == _sha(sorted(item.ir.source_text_sha256 for item in fit))
        and parent.training_receipt.get("validation_example_ids_sha256")
        == _sha(sorted(item.ir.source_text_sha256 for item in calibration))
    )
    if (graph_ids & held_ids or graph_ids & {item.ir.source_text_sha256 for item in fit}
    ...
    if args.limit:
        if args.offset + args.limit > len(held):
    ...
    body = {"schema": "aura.semantic_triadic_pair_probe.v1",
            ...
            "held_hash_order_offset": args.offset,
            "paired": paired_accuracy(*(arms[name] for name in ("incumbent", "candidate", "lesion"))),
            "arms": arms, "parent_model_trained_on_held_sources": args.source_parent is None,
