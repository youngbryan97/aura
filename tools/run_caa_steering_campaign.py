...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--trials", type=int, default=5, help="per task, per condition")
    parser.add_argument("--max-tokens", type=int, default=48)
    parser.add_argument("--alpha", type=float, default=0.0, help="0 reads the evidence file")
    parser.add_argument(
        "--evidence",
        type=Path,
    ...
    def _replaced(vector, array):
        return dataclasses.replace(vector, v=array, _v_mx=None)
    ...
    def run(name: str, prefix: str = "", steered: bool = False) -> None:
        set_alpha(alpha if steered else 0.0)
        outputs: list[str] = []
        for task in HELD_OUT_TASKS:
            for _ in range(trials):
                outputs.append(decode(prefix + task))
