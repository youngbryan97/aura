#!/usr/bin/env python3
"""Decode a verified induced neural procedure through the resident model."""
...
from core.learning.induced_neural_procedure import lower_induced_program  # noqa: E402
...
from tools.verify_induced_neural_procedure_canary import (  # noqa: E402
    verify as verify_induction_basis,
)

SCHEMA: Final = "aura.rlc.induced_neural_procedure_decode_canary.v1"
JOURNAL_SCHEMA: Final = "aura.rlc.induced_neural_procedure_decode_journal.v1"
ARMS: Final = (
    "ordinary_base",
    "matched_wire_base",
    "treatment",
...
SOURCE_PATHS: Final = (
    ...
    "tools/run_induced_neural_procedure_decode_canary.py",
...
def _source_commit(explicit: str) -> str:
    ...
    if value:
        if re.fullmatch(r"[0-9a-f]{40}", value) is None:
            raise ValueError("induced decode source commit is invalid")
        return value
    if _git("status", "--porcelain", "--untracked-files=all"):
        raise RuntimeError("induced decode canary requires clean measured source")
...
def _resident_manifest(path: Path, model_path: Path) -> dict[str, Any]:
    ...
    if active != model_path or type(manifest.get("schema_version")) is not int:
        raise ValueError("resident manifest does not select the measured model")
    return {
        "path": str(resolved),
...
def _program(payload: dict[str, Any]) -> Program:
    support = payload.get("support")
    recorded = payload.get("program")
    if not isinstance(support, list) or not support or not isinstance(recorded, dict):
        raise RuntimeError("induction basis has no frozen program")
    inputs = support[0].get("inputs")
    instructions = recorded.get("instructions")
    if not isinstance(inputs, list) or not isinstance(instructions, list):
        raise RuntimeError("induction basis program encoding is invalid")
    try:
        program = Program(
            n_inputs=len(inputs),
            instructions=tuple(
                Instruction(str(op), tuple(int(index) for index in args))
                for op, args in instructions
            ),
        )
    except (TypeError, ValueError) as exc:
        raise RuntimeError("induction basis program cannot be reconstructed") from exc
    if program.to_dict() != recorded:
        raise RuntimeError("induction basis program identity differs")
    return program


def _induction_basis(path: Path) -> tuple[dict[str, Any], Program]:
    root = path.expanduser().resolve(strict=True)
    result = json.loads((root / "result.json").read_text(encoding="utf-8"))
    stored = json.loads((root / "verification.json").read_text(encoding="utf-8"))
    replayed = verify_induction_basis(result)
    if stored != replayed or replayed.get("verified") is not True:
        raise RuntimeError("induction basis is not independently verified")
...
) -> tuple[str, SemanticNeuralCompositionDecodeState | None]:
    try:
        lowered = lower_induced_program(program, inputs)
    except (KeyError, RuntimeError, TypeError, ValueError):
        return "", None
    try:
        state = execute_composition_decode_state(
            lowered.public_workflow,
            machine=machine,
        )
        return lowered.public_workflow, state
    except (KeyError, RuntimeError, TypeError, ValueError):
        return lowered.public_workflow, None


def _states(
    program: Program,
    inputs: tuple[int, ...],
) -> tuple[str, dict[str, SemanticNeuralCompositionDecodeState | None]]:
    workflow, treatment = _decode_state(
        program,
        inputs,
        machine=SemanticNeuralMachine(),
    )
    lesion_workflow, lesion = _decode_state(program, inputs, machine=_lesion())
    left, right, denominator = inputs
    wrong_workflow, wrong_input = _decode_state(
        program,
        (left + denominator, right, denominator),
        machine=SemanticNeuralMachine(),
    )
    if not workflow or lesion_workflow != workflow or not wrong_workflow:
        raise RuntimeError("induced decode state construction failed")
    return workflow, {
        "treatment": treatment,
        "coefficient_lesion": lesion,
        "matched_wrong_input": wrong_input,
    }
...
def _wrong_state_index(
    ...
    for offset in range(1, len(states)):
        ...
        if other is not None and other.semantic_result != own.semantic_result:
            return candidate
    raise RuntimeError("induced decode cannot construct a state derangement")
...
def _prompt_tokens(tokenizer: Any, objective: str, channel: str = "") -> tuple[int, ...]:
    ...
    return tuple(int(token) for token in tokenizer.encode(rendered, add_special_tokens=False))


def _wire_prefill(tokenizer: Any) -> tuple[int, ...]:
    values = tuple(int(token) for token in tokenizer.encode("FINAL_ANSWER:{", add_special_tokens=False))
    if not values:
        raise RuntimeError("induced decode syntax prefill is empty")
    return values
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--induction-basis", type=Path, required=True)
    parser.add_argument("--out", type=Path, required=True)
    parser.add_argument("--journal", type=Path)
    parser.add_argument("--seed", type=int, default=DEFAULT_SEED + 1700)
...
def _run(args: argparse.Namespace, model_path: Path) -> int:
    if not 2 <= args.tasks <= 24 or not 24 <= args.max_tokens <= 128:
        raise ValueError("induced decode canary dimensions are invalid")
    commit = _source_commit(args.source_commit)
    manifest = _resident_manifest(args.resident_manifest, model_path)
    basis, program = _induction_basis(args.induction_basis)
    tasks = task_set(args.tasks, seed=args.seed)
    ...
    expected = [int(task.output) for task in tasks]
    journal_path = (
    ...
    if journal_path.exists():
        raise RuntimeError("induced decode journal already exists")
    ...
    journal_receipt = _append_journal(
        journal_path,
        {
    ...
    for index, (workflow, task_states, answer) in enumerate(
        zip(workflows, states, expected, strict=True)
    ):
        task_id = hashlib.sha256(workflow.encode()).hexdigest()
        treatment = task_states["treatment"]
        assert treatment is not None
        report = treatment.report
        expected_answer = {report[0]: answer}
    ...
    body = {
        ...
        "family_label_available_to_generation": False,
        "expected_answer_available_to_generation": False,
        "verifier_trace_available_to_generation": False,
    }
...
def main() -> int:
    ...
    with standalone_model_lane(
        owner_id=f"induced-procedure-decode:{args.out.name}",
