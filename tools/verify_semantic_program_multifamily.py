#!/usr/bin/env python3
"""Independently replay and recount one shared multi-family semantic campaign."""
...
def _named_paths(values: list[str]) -> dict[str, Path]:
    from tools.train_semantic_program_multifamily import _named_paths as parse
...
def main() -> int:
    ...
    try:
        ...
        bundles = {
            ...
            for name, path in _named_paths(args.bundle).items()
        }
        sources = {
            relative: hashlib.sha256(
                read_stable_bytes(_REPO_ROOT / relative, max_bytes=16 * 1024 * 1024)
            ).hexdigest()
            for relative in SEMANTIC_PROGRAM_MULTIFAMILY_VERIFICATION_SOURCES
        }
        verification = verify_semantic_program_multifamily_campaign(
            bundles,
            stored_model_payload=_load(args.model),
            stored_report=_load(args.campaign_report),
            source_sha256s=sources,
        )
        with local_internal_governed_scope(
            "semantic_program_multifamily.verification",
            domain="file_write",
        ):
            get_file_write_gateway().write_bytes(
                ...
                source="semantic_program_multifamily.verification",
            )
    except Exception as exc:  # noqa: BLE001
        print(
            json.dumps(
                {
                    "schema": "aura.semantic_program_multifamily_verifier_cli.v1",
                    "verified": False,
