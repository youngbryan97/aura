...
def main():
    ...
    # ── 8. Round-3 additions: Levi voice + frontier reasoning + tool-use +
    # self-repair + governance + Sentrux/Kame patterns + codebase walkthrough
    # + adversarial + social. Optional — only included when the round3 module
    # is importable so this builder still works on the legacy v3 corpus alone.
    round3_count = 0
    try:
        from build_round3_additions import all_round3_pairs as _r3
        round3 = list(_r3())
        all_examples.extend(round3)
        round3_count = len(round3)
        print(f"  [8] Round-3 additions:       {round3_count}")
    except Exception as exc:
        print(f"  [8] Round-3 additions: SKIPPED ({exc})")
